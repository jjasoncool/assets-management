...
export const flatpickr = (node: HTMLInputElement, options: Options = {}) => {
    ...
    function createInstance(currentOptions: Options) {
        ...
        import('flatpickr').then(({ default: fp }) => {
            ...
            if (node) {
                instance = fp(node, {
                    disableMobile: true,
                    allowInput: true,
                    ...currentOptions, // 允許外部傳入的選項
                    // 覆寫 onChange 以整合 Svelte 的數據綁定
                    onChange: (selectedDates, dateStr, inst) => {
                        // 處理使用者傳入的 onChange，可能是單一函式或函式陣列
                        if (currentOptions.onChange) {
                            if (Array.isArray(currentOptions.onChange)) {
                                for (const hook of currentOptions.onChange) {
                                    hook(selectedDates, dateStr, inst);
                                }
                            } else {
                                (currentOptions.onChange as any)(selectedDates, dateStr, inst);
                            }
                        }
    ...
    return {
        update(newOptions: Options) {
            ...
            // 找出所有變動的選項
            const changedKeys = Object.keys(newOptions).filter(
                key => (newOptions as any)[key] !== (instance?.config as any)[key]
            );

            if (changedKeys.length > 0) {
                // 使用 set 方法動態更新選項
                for (const key of changedKeys) {
                    instance.set(key as any, (newOptions as any)[key]);
                }
            }
        },
