<script lang="ts">
    ...
    /**
     * Svelte Action: 初始化 Tom Select
     * 用於將原生的 select 轉換為具備搜尋功能的下拉選單
     */
    function useTomSelect(node: HTMLSelectElement) {
        let tsInstance: any;

        // 動態載入 tom-select，避免 SvelteKit 在 Server 端渲染 (SSR) 時發生 window is not defined 錯誤
        import('tom-select').then(({ default: TomSelect }) => {
            tsInstance = new TomSelect(node, {
                create: false, // 禁止使用者自行輸入創建不存在的資產
                maxOptions: null, // 顯示所有符合搜尋的選項，不限制數量
                placeholder: '請搜尋或選擇要維護的資產...',
                searchField: ['text'], // 預設使用選項顯示的文字(資產名稱與編號)進行搜尋
                // [修正] TypeScript 要求 sortField 必須是陣列格式
                sortField: [{
                    field: 'text',
                    direction: 'asc' // 依據文字按字母順序排列
                }]
            });
        });
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="asset" name="asset" required use:useTomSelect>
                                    <option value="">請選擇要維護的資產...</option>
                                ...
                                <div class="form-text">僅顯示狀態為 Active 的資產，支援關鍵字搜尋。</div>
                            </div>
