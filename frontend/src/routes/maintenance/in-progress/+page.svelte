<script lang="ts">
    ...
    // Bootstrap 結案 Modal 實例
    let modalElement: HTMLDivElement;
    let completeModal: Modal | null = $state(null);
    let bsInstance: any = null;
    bs.subscribe((value) => (bsInstance = value));

    $effect(() => {
        if (modalElement && bsInstance) {
            completeModal = new bsInstance.Modal(modalElement);
    ...
    // 觸發詳情 Modal 開啟
    async function openDetailModal(record: any) {
                        ...
                        <tbody>
                            ...
                            {:else}
                                {#each records as record (record.id)}
                                    <tr class="cursor-pointer" onclick={() => openDetailModal(record)}>
